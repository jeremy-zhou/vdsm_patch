...
from vdsm.common import easylog
from vdsm.common import xmlparse
...
class Vm(object):
    ...
    def __init__(self, cif, params, recover=False):
        ...
        if self.xmlinfo.hostdev:
            ...
            if self.vgpuuuid:
                vmlog.info('{}'.format(self.vgpuuuid))
        
                self.xmlinfo.address.set('uuid',self.vgpuuuid)
                self.xmlinfo.metadev.set('uuid',self.vgpuuuid)

                params['xml']="{}{}".format('<?xml version="1.0" encoding="utf-8"?>',ET.tostring(self.xmlinfo.domain).decode())
                #paramlog.info('xml:{}'.format(params['xml']))
            else:
                vmlog.info('no uuid found for {}'.format(self.xmlinfo.vmname))
        else:
            vmlog.info('{} has no hostdev info'.format(self.xmlinfo.vmname))
        vmlog.info('{}'.format(self.vgpuuuid))
        vmlog.release()
